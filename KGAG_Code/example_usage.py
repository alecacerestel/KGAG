...
def example_aggregator_comparison():
    ...
    # 1. Attention-based aggregation (concat)
    print("\n1. Attention Aggregation (concat)")
    agg_concat = AttentionGroupAggregator(embedding_dim, attention_type='concat')
    group_emb, attn_weights = agg_concat(member_embeddings, item_embedding)
    print(f"   Attention weights: {attn_weights.detach().numpy()}")
    print(f"   Group embedding norm: {torch.norm(group_emb).item():.4f}")
    
    # 2. Attention-based aggregation (dot)
    print("\n2. Attention Aggregation (dot-product)")
    agg_dot = AttentionGroupAggregator(embedding_dim, attention_type='dot')
    group_emb, attn_weights = agg_dot(member_embeddings, item_embedding)
    print(f"   Attention weights: {attn_weights.detach().numpy()}")
    print(f"   Group embedding norm: {torch.norm(group_emb).item():.4f}")
